...
export default function ActivitySection({ activities }) {
  return (
    <div>
      ...
      <Card noPad>
        {activities.map((a, i) => (
          <div key={a.id || i} style={{ display: 'flex', gap: 14, padding: '14px 20px', borderBottom: i < activities.length - 1 ? `1px solid ${T.brd}` : 'none' }}>
            ...
            <div style={{ fontSize: 11, color: T.tx4, whiteSpace: 'nowrap', marginTop: 2 }}>{new Date(a.time).toLocaleString()}</div>
          </div>
        ))}
      </Card>
    </div>
